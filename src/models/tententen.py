...
class CustomModel(nn.Module):
    def __init__(self):
        ...
        self.down_conv_1 = nn.Conv3d(in_channels=5, out_channels=10, kernel_size=(9,9,9), stride=1, padding=0)     # K   x 136 x 136 x 136 ->        3*(3*9^3+1)     = 6.5K
        self.batchnorm_1 = nn.BatchNorm3d(10)

        self.down_conv_2 = nn.Conv3d(in_channels=10, out_channels=10, kernel_size=(10,10,10), stride=2, padding=0) # K   x 64  x 64  x 64  ->        44*(3*10^3+1)   = 132K
        self.batchnorm_2 = nn.BatchNorm3d(10)

        self.down_conv_3 = nn.Conv3d(in_channels=10, out_channels=10, kernel_size=(6,6,6), stride=2, padding=2)   # K   x 32  x 32  x 32  ->        32*(44*6^3+1)   = 305K
        self.batchnorm_3 = nn.BatchNorm3d(10)

        self.down_conv_4 = nn.Conv3d(in_channels=10, out_channels=10, kernel_size=(6,6,6), stride=2, padding=2)   # K   x 16  x 16  x 16  ->        76*(32*6^3+1)   = 525K
        self.batchnorm_4 = nn.BatchNorm3d(10)

        self.down_conv_5 = nn.Conv3d(in_channels=10, out_channels=10, kernel_size=(3,3,3), stride=1, padding=1)  # K   x 16  x 16  x 16  ->        288*(76*3^3+1)  = 590K
        self.batchnorm_5 = nn.BatchNorm3d(10)

        self.down_conv_6 = nn.Conv3d(in_channels=10, out_channels=10, kernel_size=(4,4,4), stride=2, padding=1) # K   x 8   x 8   x 8   ->        150*(288*4^3+1) = 2.7M
        self.batchnorm_6 = nn.BatchNorm3d(10)

        self.down_conv_7 = nn.Conv3d(in_channels=10, out_channels=512, kernel_size=(3,3,3), stride=1, padding=1) # K   x 8   x 8   x 8   ->        512*(150*3^3+1) = 2.1M
