...
class CustomModel(nn.Module):
    def __init__(self):
        ...
        self.mid_conv_1 = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=1, stride=1, padding=0)
        self.mid_conv_2 = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=1, stride=1, padding=0)
        self.mid_conv_3 = nn.Conv2d(in_channels=512, out_channels=3, kernel_size=1, stride=1, padding=0)
...
def CustomLoss(output, target):
    ...
    mid_output, position_output = output
    mid_target, position_target = target

    position_loss = ((position_output - position_target)**2).mean()
    mid_loss = ((mid_output - mid_target)**2).mean()

    print(position_loss, mid_loss)
    return position_loss + mid_loss
...
def get_data(in_fn, out_fn, mean, sdev):
    ...
    mids = np.reshape(mids, (int(num_streamlines**(1/2)), int(num_streamlines**(1/2)), 3))
