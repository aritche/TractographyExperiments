...
class CustomModel(nn.Module):
    def __init__(self):
        ...
        self.down_conv_2 = nn.Conv3d(in_channels=3, out_channels=22, kernel_size=(4,4,4), stride=2, padding=0)     # K   x 136 x 136 x 136 ->        3*(3*9^3+1)     = 6.5K
        self.batchnorm_2 = nn.BatchNorm3d(22)

        self.down_conv_3 = nn.Conv3d(in_channels=22, out_channels=44, kernel_size=(4,4,4), stride=2, padding=1) # K   x 64  x 64  x 64  ->        44*(3*10^3+1)   = 132K
        self.batchnorm_3 = nn.BatchNorm3d(44)

        self.down_conv_4 = nn.Conv3d(in_channels=44, out_channels=32, kernel_size=(4,4,4), stride=1, padding=0)   # K   x 32  x 32  x 32  ->        32*(44*6^3+1)   = 305K
        self.batchnorm_4 = nn.BatchNorm3d(32)

        # num_points*3 x 8 x 8 x 8 
        self.down_conv_5a = nn.Conv3d(in_channels=32, out_channels=num_points*3, kernel_size=(4,4,4), stride=(2,4,4), padding=(1,0,0))   # K   x 16  x 16  x 16  ->        76*(32*6^3+1)   = 525K
        self.down_conv_5b = nn.Conv3d(in_channels=32, out_channels=3, kernel_size=(4,4,4), stride=(2,4,4), padding=(1,0,0))   # K   x 16  x 16  x 16  ->        76*(32*6^3+1)   = 525K
    ...
    def forward(self, x):
        ...
        x = self.down_conv_2(x) # Output: (22, 70, 70, 70)
        x = self.batchnorm_2(x)
        x = self.relu(x)
        x = self.dropout(x)

        x = self.down_conv_3(x) # Output: (32, 35, 35, 35)
        x = self.batchnorm_3(x)
        x = self.relu(x)
        x = self.dropout(x)

        x = self.down_conv_4(x) # Output: (32, 32, 32, 32)
        x = self.batchnorm_4(x)
        x = self.tanh(x)
        x = self.dropout(x)

        p = self.down_conv_5a(x)
        p = p.view(-1, num_points*3, int(num_streamlines**(1/2)),int(num_streamlines**(1/2)))

        s = self.down_conv_5b(x)
        s = s.view(-1, 3, int(num_streamlines**(1/2)), int(num_streamlines**(1/2)))
